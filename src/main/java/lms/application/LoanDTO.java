 ...
 */
public record LoanDTO(UUID loanId, UUID userId, UUID bookId, LocalDate borrowDate, LocalDate dueDate,
	...
	public static LoanDTO fromLoan(lms.domain.Loan loan) {
		return new LoanDTO(loan.getLoanId(), loan.getUserId(), loan.getBookId(), loan.getBorrowDate(),
				loan.getDueDate(), loan.getReturnDate(), loan.isReturned(), loan.isFineApplied(), loan.isOverdue(),
				loan.getOverdueDays(), loan.calculateFine());
	}
}
